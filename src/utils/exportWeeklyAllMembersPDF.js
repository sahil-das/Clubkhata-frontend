...
/* ---------------------------------------------------------
   SHARED HEADER FUNCTION (UPDATED WITH LINKS)
   --------------------------------------------------------- */
const drawHeader = (doc, clubName, reportTitle, subtitle = "") => {
  ...
  // 👇 2. CLICKABLE LINK LOGIC
  if (WEBSITE_URL) {
    const textWidth = doc.getTextWidth(brandName);
    
    // Calculate clickable area covering Logo + Text
    const linkX = centerX - (textWidth / 2) - 5; 
    const linkY = logoY; 
    const linkWidth = textWidth + 10; 
    const linkHeight = logoSize + 8;
...
export function exportWeeklyAllMembersPDF({
  ...
  let y = drawHeader(
      doc, 
      clubName, 
      `${frequencyLabel} Collection Report`, 
      `Cycle: ${cycle.name} | Rate: ${formatCurrency(amountPerInstallment)}/${isMonthly ? 'Mo' : 'Wk'}`
  );
  ...
  doc.text("TOTAL MEMBERS", col1, summaryY);
  doc.text(`TOTAL ${unitLabel.toUpperCase()}`, col2, summaryY);
  doc.text("TOTAL COLLECTED", col3, summaryY);
  doc.text("COLLECTION RATE", col4, summaryY);

  // Row 2: Values
  const valueY = summaryY + 8;
  doc.setFontSize(11); doc.setTextColor(...COLORS.text); doc.setFont(undefined, 'normal');
  
  doc.text(`${members.length}`, col1, valueY);
  doc.text(`${totalInstallments}`, col2, valueY);
  
  doc.setTextColor(...COLORS.success);
  doc.text(`${formatCurrency(globalTotalPaid)}`, col3, valueY);

  const rateColor = Number(collectionRate) >= 100 ? COLORS.success : (Number(collectionRate) < 50 ? COLORS.danger : COLORS.primary);
  doc.setTextColor(...rateColor);
  doc.text(`${collectionRate}%`, col4, valueY);
  ...
  autoTable(doc, {
      ...
      body: tableBody,
      foot: [["Total", "", formatCurrency(globalTotalPaid), formatCurrency(globalTotalDue), ""]],
      theme: "striped",
  ...
  const footerText = "Generated by ClubKhata";
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    if (WEBSITE_URL) {
      ...
      doc.link(margin, doc.internal.pageSize.height - 13, w, 5, { url: WEBSITE_URL });
    }
