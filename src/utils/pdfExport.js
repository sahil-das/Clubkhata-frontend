...
/* =========================================================
   CONSTANTS & THEME
   ========================================================= */
// 👇 1. GLOBAL URL VARIABLE ADDED HERE
...
/* ---------------------------------------------------------
   MODERN HEADER COMPONENT (UPDATED)
   --------------------------------------------------------- */
const drawHeader = (doc, clubName, reportTitle, subtitle = "") => {
  ...
  // 👇 2. NEW CLICKABLE LINK LOGIC
  if (WEBSITE_URL) {
    // Calculate text width to define the clickable area width
    const textWidth = doc.getTextWidth(brandName);
    
    // Define the bounding box for the click (Logo + Text)
    // We center the box based on the text width (assuming text is wider than logo)
    const linkX = centerX - (textWidth / 2) - 2; // -2 for padding
    const linkY = logoY; 
    const linkWidth = textWidth + 4; // +4 for padding
    const linkHeight = logoSize + 8; // Covers logo (12) + gap + text height

    // Add the link annotation
    doc.link(linkX, linkY, linkWidth, linkHeight, { url: WEBSITE_URL });
  }
...
const drawFooter = (doc) => {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    // 👇 Optional: Make footer text clickable too
    const footerText = `Generated by ClubKhata • ${new Date().toLocaleDateString()}`;
    doc.text(footerText, 14, pageHeight - 10);
    if(WEBSITE_URL) {
        const w = doc.getTextWidth("Generated by ClubKhata");
        doc.link(14, pageHeight - 13, w, 5, { url: WEBSITE_URL });
    }
