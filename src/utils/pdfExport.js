...
/* ---------------------------------------------------------
   MODERN HEADER COMPONENT (UPDATED LAYOUT)
   --------------------------------------------------------- */
const drawHeader = (doc, clubName, reportTitle, subtitle = "") => {
  ...
  const logoSize = 12; 
  const logoY = 10; // Slightly higher to fit text below
  // Position logo so text stays within margin. 
  // Adjusted logoX to give space for centered text below it.
  const logoX = pageWidth - margin - 20; 
  const centerX = logoX + (logoSize / 2); // Center point for alignment
  ...
  doc.setFont("helvetica", "bold");
  // 👈 Placed below icon, centered relative to icon
  doc.text("ClubKhata", centerX, logoY + logoSize + 6, { align: "center" });


  // --- LEFT SIDE: REPORT TITLES ---
