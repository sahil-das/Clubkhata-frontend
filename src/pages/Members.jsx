  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        {/* Search */}
        <div className="bg-white dark:bg-slate-900 p-2 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-2 flex-1">
          <Search className="text-slate-400 dark:text-slate-500 ml-2" size={20} />
          <input 
            type="text"
            placeholder="Search by name..."
            className="flex-1 outline-none text-sm p-2 text-slate-700 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-600 bg-transparent"
            value={searchTerm}
        ...
        {/* View Toggle Buttons */}
        <div className="bg-white dark:bg-slate-900 p-1 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-1 shrink-0">
          <button 
          ...
          {viewMode === "grid" && (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredMembers.map((member) => {
                ...
                return (
                  <div key={member.membershipId} className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all relative">
                    <div className="flex justify-between items-start">
                       <Wrapper {...wrapperProps}>
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold shrink-0 ${
                            member.role === 'admin' 
                              ? "bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400 ring-2 ring-indigo-500 ring-offset-2 dark:ring-offset-slate-900" 
                          ...
                          <div className="min-w-0">
                            <h3 className="font-bold text-slate-800 dark:text-slate-100 truncate flex items-center gap-1">
                                {member.name} 
                                {isAdmin && <ChevronRight size={14} className="text-slate-300 dark:text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity" />}
                            </h3>
                       ...
                       {isAdmin && (
                         <div className="flex gap-1">
                            <button onClick={() => initiateRoleToggle(member)} className={`p-2 rounded-lg transition-colors ${member.role === 'admin' ? "bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400" : "text-slate-300 dark:text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>
                              <ShieldCheck size={18} />
                            </button>
                            <button onClick={() => initiateDelete(member.membershipId)} className="p-2 text-slate-300 dark:text-slate-600 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                              <Trash2 size={18} />
                            </button>
                         </div>
                       )}
                    ...
                    {/* Mobile Quick Actions (Grid Only) */}
                    {isAdmin && member.phone && (
                       <div className="mt-4 pt-3 border-t border-slate-50 dark:border-slate-800 grid grid-cols-2 gap-3">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                    <tr>
                      <th className="px-6 py-4 font-bold text-slate-500 dark:text-slate-400 uppercase text-xs">Member</th>
                      <th className="px-6 py-4 font-bold text-slate-500 dark:text-slate-400 uppercase text-xs">Role</th>
                      {isAdmin && <th className="px-6 py-4 font-bold text-slate-500 dark:text-slate-400 uppercase text-xs">Contact</th>}
                      {isAdmin && <th className="px-6 py-4 font-bold text-slate-500 dark:text-slate-400 uppercase text-xs text-right">Actions</th>}
                    </tr>
                  </thead>
                          ...
                          <td className="px-6 py-4">
                             <Wrapper {...wrapperProps}>
                                ...
                                <div>
                                   ...
                                   <div className="text-slate-400 dark:text-slate-500 text-xs">{member.email}</div>
                                </div>
                             </Wrapper>
                          </td>
                          <td className="px-6 py-4">
                            ...
                            <td className="px-6 py-4 text-slate-500 dark:text-slate-400 font-medium">
                               {member.phone ? (
                                 <div className="flex gap-3">
                                   <a href={`tel:${member.phone}`} className="hover:text-slate-800 dark:hover:text-slate-200" title="Call">
                                     {member.phone}
                                   </a>
                               ...
                               ) : <span className="text-slate-300 dark:text-slate-600 italic">No phone</span>}
                            </td>
                          )}
                          {isAdmin && (
                            <td className="px-6 py-4 text-right">
                               <div className="flex items-center justify-end gap-2">
                                  <button onClick={() => initiateRoleToggle(member)} className={`p-2 rounded-lg transition-colors ${member.role === 'admin' ? "bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400" : "text-slate-300 dark:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-300"}`} title="Toggle Role">
                                    <ShieldCheck size={18} />
                                  </button>
                                  <button onClick={() => initiateDelete(member.membershipId)} className="p-2 text-slate-300 dark:text-slate-600 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Delete Member">
                                    <Trash2 size={18} />
                                  </button>
                               </div>
                            </td>
